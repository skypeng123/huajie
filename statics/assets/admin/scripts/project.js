AM.projects = {
    ...
    loadSWFupload : function(){
        window.onload = function () {
            this.swfu = new SWFUpload({
                // Backend Settings
                upload_url: admin_url+"projects/upload",
    ...
    loadLists : function(){
        this.table = $('#item_list').DataTable({
            ajax: {
                //指定数据源
                url: admin_url+"projects/getlist"
            },
            language: {
                "search": "过滤记录:",
            ...
            "columnDefs": [
                ...
                {
                    "render": function(data, type, row, meta) {
                        return '<td><div class=" btn-group-sm btn-group-solid" ><a role="button" class="btn blue btn-sx edit-btn" href="'+admin_url+'projects/edit/'+row['id']+'"> <i class="fa fa-pencil"></i> 修改 </a>  <a class="btn red btn-sx del-btn" role="button" data-id="'+row['id']+'"> <i class="fa fa-remove"></i>  删除 </a></div><td>';
    ...
    saveItem : function(){
        ...
        $.ajax({
            "url":admin_url+"projects/save",
            "data":{id:id,title:title,order_num:order_num,catid:catid,state:state,content:content,recommend:recommend,pic:pic},
            "dataType":"json",
            "type":"POST",
            "success":function (rdata) {
                if(rdata.code == 200){
