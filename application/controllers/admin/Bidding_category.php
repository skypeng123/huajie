...
class Bidding_category extends Admin_Controller {
	...
	public function index() {
        ...
        $this -> load -> view('admin/bidding_category.html',$data);
	}
	...
	public function getlist() {
		...
		$datalist = $this -> bidding_category_model -> fetch_all($condition);
        $data = [];
	...
	public function getinfo() {
		...
		$this -> load -> model('bidding_category_model');
		$info = $this -> bidding_category_model -> fetch_info($id);
		if(!$info){
	...
	public function save() {
		...
		$info = array();
		if (!empty($dataset['id'])) {		
			$info = $this -> bidding_category_model -> fetch_info($dataset['id']);
		} else{
            $dataset['addtime'] = time();
        }
		$id = $this -> bidding_category_model -> save($dataset);
		if ($id) {
			if (!empty($dataset['id'])) {		
				$event = "招标分类 [" . $id . "] 已被修改";
			} else {
	...
	public function del() {
		...
		if ($ids) {
			...
			foreach ($arr as $id) {
				$info = $this -> bidding_category_model -> fetch_info($id);
				$res = $this -> bidding_category_model -> delete($id);
				if ($res) {
					$id_arr[] = $id;
					$delete_data[] = $info;
				}
			...
			if (!empty($id_arr)) {
				$this -> write_log("招标分类 [" . join(',', $id_arr) . "] 已被删除", $delete_data);
				$this -> return_data(200, "招标分类删除成功");
			}
		}
		$this -> return_data(400, "招标分类删除失败");
	}
