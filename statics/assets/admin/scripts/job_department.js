AM.job_department = {
    ...
    showModal : function(type,data){
        ...
        if(type == 'add'){
            ...
            myform.find('.modal-title').html('添加部门');
            myform.find('input[name=state]').eq(0).prop('checked',true);
            myform.modal('show');
        }else if(type == 'edit'){
            myform.find('.modal-title').html('修改部门');
    ...
    loadLists : function(){
        this.table = $('#item_list').DataTable({
            ajax: {
                //指定数据源
                url: admin_url+"job_department/getlist"
            },
            language: {
                "search": "过滤记录:",
    ...
    editItem : function(id){
        ...
        that.loading('show');
        $.ajax({
            "url":admin_url+"job_department/getinfo",
            "data":{id:id},
            "dataType":"json",
            "type":"POST",
    ...
    delItem : function(ids){
        ...
        bootbox.confirm({
            ...
            callback: function (result) {
                ...
                that.loading('show');
                $.ajax({
                    "url":admin_url+"job_department/del",
                    "data":{ids:ids},
                    "dataType":"json",
                    "type":"POST",
                    "success":function (rdata) {
    ...
    saveItem : function(){
        ...
        $.ajax({
            "url":admin_url+"job_department/save",
