...
class News extends Admin_Controller {
	...
	public function index() {
		...
		$this -> load -> view('admin/news_list.html',$data);
	}
    ...
    public function add() {
        ...
        $category_list = $this->news_category_model->fetch_all($con);
        $data['category_list'] = $category_list;

        $data['header'] = $this -> load-> view('admin/header.html',$data,true);
        $data['footer'] = $this -> load-> view('admin/footer.html',$data,true);
        $data['sidebar'] = $this -> load-> view('admin/sidebar.html',$data,true);
        $this -> load -> view('admin/news_add.html',$data);
    }
    ...
    public function edit($id) {
        ...
        $data['news'] = $this->news_model->fetch_info($id);


        $data['header'] = $this -> load-> view('admin/header.html',$data,true);
        $data['footer'] = $this -> load-> view('admin/footer.html',$data,true);
        $data['sidebar'] = $this -> load-> view('admin/sidebar.html',$data,true);
        $this -> load -> view('admin/news_edit.html',$data);
    }
	...
	public function getlist() {
		...
		$datalist = $this -> news_model -> fetch_all($condition);
		$catids = get_array_col($datalist, 'catid');
		if(!empty($catids)){
			$this -> load -> model('news_category_model');
			$catlist = $this->news_category_model->fetch_list_by_ids($catids);
	...
	public function getinfo() {
		...
		$this -> load -> model('news_model');	
		$info = $this -> news_model -> fetch_info($id);
		if(!$info){
			$this -> return_data(400, '数据不存在');
		}

		//$info = $this -> news_model ->fetch_info($uid);
		if (!empty($info)) {
	...
	public function save() {
		...
		$dataset['subtitle'] = $this -> input -> trim_post('subtitle');
		$dataset['origin'] = $this -> input -> trim_post('origin');
		$dataset['author'] = $this -> input -> trim_post('author');
		$dataset['views'] = $this -> input -> post_int('views');
		...
		$dataset['keywords'] = $this -> input -> trim_post('keywords');
