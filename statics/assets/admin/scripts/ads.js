AM.ads = {
    ...
    loadSWFupload : function(){
        window.onload = function () {
            this.swfu = new SWFUpload({
                // Backend Settings
                upload_url: admin_url+"ads/upload",
    ...
    loadLists : function(){
        this.table = $('#item_list').DataTable({
            ajax: {
                //指定数据源
                url: admin_url+"ads/getlist"
            },
            language: {
                "search": "过滤记录:",
                "searchPlaceholder": "广告类型/标题",
            ...
            "columnDefs": [
                ...
                },
                {
                    "render": function(data, type, row, meta) {
                        if(data !=''){
                            str = '<a href="'+data+'" target="_blank"><img src="'+data+'" height="30px"></a>';
                        }else{
                            str = '';
                        }
                        return '<td>'+str+'<td>';
                ...
                {
                    "render": function(data, type, row, meta) {
                        return '<td><div class=" btn-group-sm btn-group-solid" ><a role="button" class="btn blue btn-sx edit-btn" href="'+admin_url+'ads/edit/'+row['id']+'"> <i class="fa fa-pencil"></i> 修改 </a>  <a class="btn red btn-sx del-btn" role="button" data-id="'+row['id']+'"> <i class="fa fa-remove"></i>  删除 </a></div><td>';
    ...
    saveItem : function(){
        ...
        description=saveForm.find("textarea[name=description]").val();
        pic = $('#thumbnails img').attr("src");

        $.ajax({
            "url":admin_url+"ads/save",
            "data":{id:id,title:title,order_num:order_num,catid:catid,url:url,state:state,pic:pic,description:description},
            "dataType":"json",
            "type":"POST",
            "success":function (rdata) {
                if(rdata.code == 200){
                    ...
                    setTimeout(function(){
                        window.location.href=admin_url+'ads'
                    },1000);
