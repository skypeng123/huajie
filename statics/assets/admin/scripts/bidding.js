AM.bidding = {
    ...
    loadSWFupload : function(){
        ...
        this.swfu = new SWFUpload({
                // Backend Settings
                upload_url: admin_url+"bidding/upload",
                post_params: {"sessid": sessid},
                file_post_name: "file",

                // File Upload Settings
                file_size_limit : "20 MB",	// 2MB
                file_types : "*.doc;*.docx;*.pdf;*.rar;*.zip",
                ...
                button_width: 150,
                button_height: 35,
                button_text : '<span class="upbtn">上传文件</span><span class="buttonSmall"> (大小限制:10M)</span>',
        ...
        this.table = $('#item_list').DataTable({
            ...
            "columnDefs": [
                ...
                },
                {
                    "render": function(data, type, row, meta) {
                        if(data > 0){
                            str = '<a href="'+admin_url+'bidding_submit?bidding_id='+row['id']+'">'+data+'</a>';
                        }else{
                            str = data;
                        }
                        return '<td>'+str+'<td>';
                ...
                {
                    "render": function(data, type, row, meta) {
                        return '<td><div class=" btn-group-sm btn-group-solid" ><a role="button" class="btn blue btn-sx edit-btn" href="'+admin_url+'bidding/edit/'+row['id']+'"> <i class="fa fa-pencil"></i> 修改 </a>  <a class="btn red btn-sx del-btn" role="button" data-id="'+row['id']+'"> <i class="fa fa-remove"></i>  删除 </a></div><td>';
    ...
    saveItem : function(){
        ...
        var saveForm = $('#biddingForm');
        id=saveForm.find("input[name=id]").val();
        title=saveForm.find("input[name=title]").val();
        start_date=saveForm.find("input[name=start_date]").val();
        end_date=saveForm.find("input[name=end_date]").val();
        catid=saveForm.find("select[name=catid]").val();
        state=saveForm.find("input[name=state]:checked").val();

        fileurl=saveForm.find("input[name=fileurl]").val();
        filename=saveForm.find("input[name=filename]").val();
        order_num=saveForm.find("input[name=order_num]").val();
        content = that.ue.getContent();

        $.ajax({
            "url":admin_url+"bidding/save",
            "data":{id:id,title:title,start_date:start_date,end_date:end_date,catid:catid,fileurl:fileurl,filename:filename,state:state,content:content,order_num:order_num},
            "dataType":"json",
            "type":"POST",
            "success":function (rdata) {
                if(rdata.code == 200){
                    ...
                    setTimeout(function(){
                        window.location.href=admin_url+'bidding'
                    },1000);
