AM.bidding_submit = {
    ...
    showModal : function(type,data){
        ...
        console.log(data);
        if(type == 'view'){
            myform.find('.modal-title').html('查看投标');
            myform.find('input[name="id"]').val(data.id);
            myform.find('input[name="company_name"]').val(data.company_name);
            myform.find('input[name="mobile"]').val(data.mobile);
            myform.find('input[name="contact"]').val(data.contact);
            myform.find('textarea[name="remark"]').val(data.remark);
            myform.find('input[name="addtime"]').val(data.addtime);
            myform.find('.file').html('<a href="'+data.fileurl+'" target="_blank">'+data.filename+'</a>');
            myform.modal('show');
        }
    ...
    loadLists : function(){
        this.table = $('#item_list').DataTable({
            ajax: {
                //指定数据源
                url: admin_url+"bidding_submit/getlist"
            },
            language: {
                "search": "过滤记录:",
                "searchPlaceholder": "公司名/联系人/手机号",
    ...
    viewItem : function(id){
        ...
        that.loading('show');
        $.ajax({
            "url":admin_url+"bidding_submit/getinfo",
            "data":{id:id},
            "dataType":"json",
            "type":"POST",
    ...
    delItem : function(ids){
        ...
        bootbox.confirm({
            ...
            callback: function (result) {
                ...
                that.loading('show');
                $.ajax({
                    "url":admin_url+"bidding_submit/del",
                    "data":{ids:ids},
                    "dataType":"json",
                    "type":"POST",
                    "success":function (rdata) {
